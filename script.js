        ...
        /**
 * وظائف القرآن - نسخة محسنة
 * تحميل بيانات السور وعرض آية اليوم
 */
async function loadQuranData() {
    try {
        // تحميل قائمة السور مع استخدام HTTPS للأمان
        const surahsResponse = await fetch('https://api.alquran.cloud/v1/surah');
        
        if (!surahsResponse.ok) {
            throw new Error(`فشل في تحميل بيانات السور: ${surahsResponse.status}`);
        }
        ...
        if (surahsData.code === 200) {
            ...
            throw new Error(`فشل في تحميل بيانات السور: ${surahsData.status}`);
        }
        
        // تحميل آية اليوم - اختيار سورة وآية عشوائية
        const randomSurah = Math.floor(Math.random() * 114) + 1;
        const ayahResponse = await fetch(`https://api.alquran.cloud/v1/ayah/${randomSurah}:1/ar`);
        
        if (!ayahResponse.ok) {
            throw new Error(`فشل في تحميل آية اليوم: ${ayahResponse.status}`);
        }
        
        const ayahData = await ayahResponse.json();
        
        if (ayahData.code === 200) {
            const ayahElement = document.getElementById('ayah-of-day');
            if (ayahElement) {
            ...
            throw new Error(`فشل في تحميل آية اليوم: ${ayahData.status}`);
        }
        
    } catch (error) {
        console.error('خطأ في تحميل بيانات القرآن:', error);
        
        const surahsElement = document.getElementById('quran-surahs');
        if (surahsElement) {
            surahsElement.innerText = 'حدث خطأ أثناء تحميل بيانات القرآن';
        }
        
        const ayahElement = document.getElementById('ayah-of-day');
        if (ayahElement) {
            ayahElement.innerText = 'حدث خطأ أثناء تحميل آية اليوم';
        }
    }
}

/**
 * عرض قائمة السور في عنصر select
 * @param {Array} surahs - مصفوفة تحتوي على بيانات السور
 */
function displaySurahs(surahs) {
    const container = document.getElementById('quran-surahs');
    if (!container) {
        console.error('لم يتم العثور على عنصر quran-surahs');
        return;
    }
    ...
    defaultOption.value = '';
    defaultOption.innerText = 'اختر سورة';
...
// بدء تحميل بيانات القرآن عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', loadQuranData);
